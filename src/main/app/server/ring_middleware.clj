(ns app.server.ring-middleware
  "Ring middleware setup following facade pattern."
  (:require
    [clojure.string :as str]
    [com.fulcrologic.fulcro.server.api-middleware :as server]
    [com.fulcrologic.rad.type-support.date-time :as dt]
    [hiccup.page :refer [html5]]
    [mount.core :refer [defstate]]
    [ring.middleware.defaults :refer [wrap-defaults]]
    [ring.util.response :as resp]
    [app.server.config :as config]
    [app.server.parser :as parser]))

(defn index [csrf-token]
  (html5
    [:html {:lang "en"}
     [:head {:lang "en"}
      [:title "Datalevin RAD Test Application"]
      [:meta {:charset "utf-8"}]
      [:meta {:name "viewport" :content "width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no"}]
      [:link {:href "https://cdn.jsdelivr.net/npm/fomantic-ui@2.9.4/dist/semantic.min.css"
              :rel  "stylesheet"}]
      [:link {:rel "shortcut icon" :href "data:image/x-icon;," :type "image/x-icon"}]
      [:script (str "var fulcro_network_csrf_token = '" csrf-token "';")]]
     [:body
      [:div#app
       [:div.ui.active.dimmer
        [:div.ui.large.text.loader "Loading"]]]
      [:script {:src "/js/main/main.js"}]]]))

(defn wrap-api [handler uri]
  (fn [request]
    (if (= uri (:uri request))
        (server/handle-api-request (:transit-params request)
          (fn [query]
            (dt/with-timezone "America/Phoenix"
              (parser/parser {:ring/request request}
                query))))
        (handler request))))

(def not-found-handler
  (fn [req]
    {:status 404
     :body   {}}))

(defn wrap-html-routes [ring-handler]
  (fn [{:keys [uri anti-forgery-token] :as req}]
    (if (or (str/starts-with? uri "/api")
            (str/starts-with? uri "/js"))
        (ring-handler req)
        (-> (resp/response (index anti-forgery-token))
            (resp/content-type "text/html")))))

(defstate middleware
  :start
  (let [defaults-config (:ring.middleware/defaults-config config/config)]
    (-> not-found-handler
        (wrap-api "/api")
        (server/wrap-transit-params {})
        (server/wrap-transit-response {})
        (wrap-html-routes)
        (wrap-defaults defaults-config))))
